...
class CotsDetector(AbstractCotsDetector):
    ...
    def callProgram(self, survey_path):
        # run the process
        script = "$REEFSCAN_HOME/reefscan_desktop_data_manager/linux_scripts/cots-detector-two-cameras.sh"
        ...
        cmd = f'source ~/.reefscan_env && bash {script} "{input_path}" "{output_path}"'
